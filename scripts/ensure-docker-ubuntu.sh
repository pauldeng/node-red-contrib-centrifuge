...
if command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1; then
  ...
  echo "Fix Docker daemon access, or open a new shell after joining the docker group, before running the Docker test tiers (npm run test:docker)." >&2
  exit 1
fi
...
if ! has_working_docker; then
  ...
  echo "Add the user to the docker group and open a new login shell." >&2
  exit 1
fi
