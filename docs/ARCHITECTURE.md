...
| [connection.js](../lib/connection.js)                        | One SDK client, connection status, shared subscription registry (each channel entry records a `stream` or map subscription type), readiness waiters and command execution.                                                                                                                        |
| [centrifuge-in.js](../nodes/centrifuge-in.js)                | Client-side or server-side subscription consumer; turns publications, selected join/leave events, and map `sync`/`update` events into flow messages.                                                                                                                                              |
| [centrifuge-out.js](../nodes/centrifuge-out.js)              | Evaluates a channel (and, for the map modes, a key) per input, prepares its payload, issues `publish`/`map_publish`/`map_remove` and forwards the original message after acknowledgement. Pending input cancellation and settlement is shared with `centrifuge-request.js` via `command-node.js`. |
...
- Consumers sharing a channel use one SDK subscription of one subscription type (`stream`, or a map type: `map`, `map_clients`, `map_users`), created through the matching SDK factory (`newSubscription` or `newMapSubscription`/`newMapClientsSubscription`/`newMapUsersSubscription`). A `stream` subscription always requests join/leave (the server only sends them where the namespace enables them) and events are filtered per consumer, so opting in never recreates the subscription or drops publications; a map subscription forwards the SDK's `sync` (full snapshot) and `update` (per-key) events to every consumer instead, and does not request join/leave. A second consumer of the same channel with a different subscription type is rejected as `INVALID_CONFIG`. Server-side/client-side overlap is `CONFIG_CONFLICT`; server-side subscriptions to map channels are not supported. Disposing a map subscription calls the SDK's `removeMapSubscription` rather than `removeSubscription`.
- Each shared map subscription owns one snapshot cache, replaced on `sync` and updated or deleted by key on `update`. New consumers receive a copy, including an empty snapshot, without restarting the SDK subscription. Keep the last known state across reconnects because successful SDK recovery emits only incremental updates; a full resync replaces it. Destroying the entry clears the cache.
