  ...
  try {
    ...
    await writeFile(
      ...
      `module.exports = ${JSON.stringify({
        ...
        ...settings,
      })};\n${settingsModule ? `Object.assign(module.exports, require(${JSON.stringify(settingsModule)}));\n` : ""}`,
    );
    child = spawn(process.execPath, [require.resolve("node-red/red.js"), "-u", userDir, "-s", settingsFile], {
      stdio: ["ignore", "pipe", "pipe"],
      env: { ...process.env, NODE_ENV: "test", ...env }, // e.g. NODE_EXTRA_CA_CERTS for wss:// with a private CA
    });
    child.on("error", (err) => {
    ...
    const deploy = async (flows, type = "full") => {
      ...
      // "full" logs "Started flows"; "flows"/"nodes" deploys log "Started modified flows|nodes". A modified deploy
      // that changes nothing logs neither and would time out here: change something or use type "full".
