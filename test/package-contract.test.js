...
test("package metadata promises hold", (t) => {
  ...
  if (!fs.existsSync(lockPath)) {
    t.diagnostic("no package-lock.json yet");
    return;
  }
...
test("CI matrix tests the declared Node floor", () => {
  const ci = path.join(ROOT, ".github/workflows/ci.yml");
  const floor = pkg.engines.node.match(/>=\s*(\d+)/)[1];
  assert.match(
    fs.readFileSync(ci, "utf8"),
    new RegExp(`node:\\s*\\[[^\\]]*"${floor}"`),
    `ci.yml matrix must include Node ${floor}`,
  );
});

test("release workflow, if present, uses OIDC only (publishing is out of scope for now)", (t) => {
  const rel = path.join(ROOT, ".github/workflows/release.yml");
  if (!fs.existsSync(rel)) {
    t.skip("no release workflow; publishing is not in scope");
    return;
  }
