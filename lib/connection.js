...
const SUB_TYPES = new Set(["stream", "map", "map_clients", "map_users"]);
// Map subscriptions (Centrifugo >= 6.8, experimental): one SDK factory per subscription type.
const MAP_FACTORY = {
  map: "newMapSubscription",
  map_clients: "newMapClientsSubscription",
  map_users: "newMapUsersSubscription",
};
...
function createConnection(opts) {
  ...
  // A consumer callback runs inside the SDK's emit; a throw there would skip sibling consumers and escape into the
  // WebSocket handler. Exception text may contain payloads or secrets, so log only a local diagnostic.
  const safe = (fn, ...args) => {
    if (typeof fn !== "function") return;
    try {
      fn(...args);
    } catch {
      log.warn("centrifuge consumer callback failed");
    }
  };
  ...
  const publishConnection = (status) => {
    if (closing) return;
    connection = status;
    for (const fn of statusListeners) safe(fn, status);
    for (const entry of registry.values()) entry.push();
    for (const c of serverConsumers) safe(c.onStatus, serverStatus(c));
  };
  ...
  // Map entries as the SDK delivers them (probe map): data is absent for removals and presence maps.
  const mapEntry = ({ key, data, offset, info }) => ({
    key,
    ...(data !== undefined && { data }),
  ...
  const eventContext = (event, ctx) => {
    ...
    } else if (event === "sync") out.entries = ctx.entries.map(mapEntry);
    else if (event === "update") Object.assign(out, mapEntry(ctx), ctx.removed === true ? { removed: true } : {});
  ...
  const handlers = {
    ...
    subscribing: (ctx) => {
      ...
      for (const c of serverConsumers) if (c.channel === ctx.channel) safe(c.onStatus, serverStatus(c));
    },
    subscribed: (ctx) => {
  ...
  function fanOutServer(event, ctx) {
    ...
    for (const c of serverConsumers)
      if ((!c.channel || c.channel === ctx.channel) && (event === "publication" || c.joinLeave)) safe(c.onEvent, out);
  }
  for (const [name, fn] of Object.entries(handlers)) client.on(name, fn);

  // One SDK subscription per channel. A stream subscription always requests join/leave (the server sends them only
  // where the namespace enables them) so consumers can opt in or out without recreating the subscription; recreating
  // loses publications. A map subscription delivers `sync` (full snapshot) and `update` (one key) events instead.
  function createEntry(channel, { auth, type }) {
    const entry = { channel, auth, type, consumers: new Set(), status: STATUS.subscribing, sub: null };
    const stream = type === "stream";
    // Retain the last known map across reconnects: successful SDK recovery emits only updates.
    // null means no initial sync yet; an empty Map is a valid snapshot to replay.
    let mapState = null;
    entry.replay = (consumer) => {
      if (closing || mapState === null) return;
      safe(consumer.onEvent, { event: "sync", channel, entries: structuredClone([...mapState.values()]) });
    };
    entry.combined = () => (connection !== STATUS.connected ? connection : entry.status);
    entry.push = () => {
      if (closing) return;
      const s = entry.combined();
      for (const c of entry.consumers) safe(c.onStatus, s);
    };
    entry.attach = () => {
      const options = stream ? { joinLeave: true } : {};
      if (entry.auth === "hmac")
        options.getToken = async (ctx) =>
          subscriptionToken({ secret: cfg.secret, sub: cfg.user, channel: ctx.channel, ttl: cfg.ttl });
      const sub = stream ? client.newSubscription(channel, options) : client[MAP_FACTORY[type]](channel, options);
      ...
      const deliver = (event) => (ctx) => {
        ...
        // Own a separate copy so downstream mutation cannot corrupt future snapshots.
        if (event === "sync") mapState = new Map(structuredClone(out.entries).map((item) => [item.key, item]));
        else if (event === "update" && mapState !== null) {
          if (out.removed) mapState.delete(out.key);
          else mapState.set(out.key, structuredClone(mapEntry(out)));
        }
        for (const c of entry.consumers)
          if ((event !== "join" && event !== "leave") || c.joinLeave) safe(c.onEvent, out);
      };
      const owned = new Map();
      ...
      if (stream) {
        ...
        // A map subscription also emits `publication` for every change; only `update` carries the key (probe map).
        on("sync", deliver("sync"));
        on("update", deliver("update"));
      }
      on("subscribing", () => {
    ...
    entry.destroy = () => {
      ...
      if (!sub) return;
      if (stream)
        client.removeSubscription(sub); // unsubscribes internally; no separate unsubscribe() call
      else client.removeMapSubscription(sub);
    };
    entry.attach();
    return entry;
  }

  // subscribe(channel, { onEvent, onStatus }, { joinLeave, subscriptionAuth, type }) -> dispose()
  // type: "stream" (default) or a map type; every consumer of one channel must use the same type and auth.
  function subscribe(channel, consumer, { joinLeave = false, subscriptionAuth = "none", type = "stream" } = {}) {
    ...
    if (!SUB_TYPES.has(type)) throw coded("INVALID_CONFIG", "type must be stream, map, map_clients or map_users");
    ...
    if (entry && entry.type !== type)
      throw coded("INVALID_CONFIG", "subscription type differs from the existing subscription");
    if (!entry) {
      entry = createEntry(channel, { auth: subscriptionAuth, type });
      registry.set(channel, entry);
    }
    consumer.joinLeave = joinLeave === true;
    entry.consumers.add(consumer);
    safe(consumer.onStatus, entry.combined());
    entry.replay(consumer);
    let disposed = false;
    return () => {
      if (disposed) return;
  ...
  function onServerSide(consumer) {
    ...
    safe(consumer.onStatus, serverStatus(consumer));
