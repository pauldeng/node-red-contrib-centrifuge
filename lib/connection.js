...
function createConnection(opts) {
  ...
  // One SDK subscription per channel. It always requests join/leave (the server sends them only where the namespace
  // enables them) so consumers can opt in or out without recreating the subscription; recreating loses publications.
  function createEntry(channel, { auth }) {
    const entry = { channel, auth, consumers: new Set(), status: STATUS.subscribing, sub: null };
    ...
    entry.attach = () => {
      ...
      const deliver = (event) => (ctx) => {
        ...
        for (const c of entry.consumers) if (event === "publication" || c.joinLeave) safe(c.onEvent, out);
      };
      const owned = new Map();
  ...
  function subscribe(channel, consumer, { joinLeave = false, subscriptionAuth = "none" } = {}) {
    ...
    if (!entry) {
      entry = createEntry(channel, { auth: subscriptionAuth });
