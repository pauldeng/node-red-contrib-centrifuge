...
// Snapshot and size-check a complete command, including commands without a data field.
function prepareRequest(method, params, maxMessageSize) {
  let text;
  try {
    text = JSON.stringify({ [method]: params, id: ID_ALLOWANCE }, rejectBinary);
  } catch {
  ...
  return { params: parsed[method], bytes };
}
...
function prepareCommand(method, params, data, maxMessageSize) {
  ...
  const prepared = prepareRequest(method, { ...params, data }, maxMessageSize);
  if (!("data" in prepared.params)) throw coded("INVALID_MESSAGE", "payload produces no JSON value");
  return { data: prepared.params.data, bytes: prepared.bytes };
}

module.exports = { prepareCommand, prepareRequest };
