...
[![npm version](https://img.shields.io/npm/v/@pauldeng/node-red-contrib-centrifuge.svg)](https://www.npmjs.com/package/@pauldeng/node-red-contrib-centrifuge)
[![npm downloads](https://img.shields.io/npm/dm/@pauldeng/node-red-contrib-centrifuge.svg)](https://www.npmjs.com/package/@pauldeng/node-red-contrib-centrifuge)
[![CI](https://github.com/pauldeng/node-red-contrib-centrifuge/actions/workflows/ci.yml/badge.svg)](https://github.com/pauldeng/node-red-contrib-centrifuge/actions/workflows/ci.yml)
[![OpenSSF Scorecard](https://api.scorecard.dev/projects/github.com/pauldeng/node-red-contrib-centrifuge/badge)](https://scorecard.dev/viewer/?uri=github.com/pauldeng/node-red-contrib-centrifuge)
[![License: MIT](https://img.shields.io/npm/l/@pauldeng/node-red-contrib-centrifuge.svg)](LICENSE)
...
- `map` — a map subscription to **Channel**: a server-owned key/value collection on a namespace configured with `subscription_type: "map"`. Join/leave does not apply; a **subscription token** still applies as for `subscribe`.
- `map_clients` / `map_users` — a map subscription to a server-managed presence map, fed by another namespace's `map_clients_presence_channel_prefix` / `map_users_presence_channel_prefix`. Join/leave does not apply.

A channel cannot be both in the server node's **Channels** list and used by a `subscribe`-mode node on the same server: that combination is rejected as a conflicting subscription. Server-side subscriptions (the server node's **Channels** list, or `server` mode) do not support map channels. Two `centrifuge in` nodes on the same channel must use the same **Mode**; a mismatch shows `invalid config: subscription type differs from the existing subscription`.

Output message shape for `subscribe` and `server` modes:
...
Output message shape for `map`, `map_clients` and `map_users` modes — first the current snapshot, then one message per key change (also resent as a snapshot after a full resync):

- On snapshot: `msg.payload` is the array of current entries, each `{ key, data?, offset?, info? }`; `msg.centrifuge = { event: "sync", channel }`.
- On a key change: `msg.payload` is the entry's data, the `ClientInfo` for a `map_clients` join, or `null` for a removal and for a `map_users` join; `msg.centrifuge = { event: "update", channel, key, removed: true (only for removals), offset?, info? }`.
...
Map subscriptions are marked experimental by Centrifugo and may change; see [map subscriptions](https://centrifugal.dev/docs/server/map_subscriptions).
...
| `INVALID_CONFIG`    | Status `invalid config: ...`.                                                         | The server configuration's settings (URL, auth, ranges) failed validation, or two `centrifuge in` nodes on the same channel use a different **Mode** (subscription type).                                                                                                                                                                                                                                                                                      | Fix the configuration and redeploy.                                                             |
...
| `PERMISSION_DENIED` | Subscribe, publish, or a `centrifuge request` is refused by the server.               | The channel's namespace lacks `allow_subscribe_for_client` / `allow_publish_for_client` (or `_for_subscriber`); for `centrifuge request`, `allow_history_for_client` or `allow_presence_for_client`. For a map channel: the namespace lacks `allow_publish_for_client` / `allow_remove_for_client` for `map_publish` / `map_remove`, a plain `publish` targets a map channel, or a subscription's **Mode** does not match the namespace's `subscription_type`. | Grant the permission on the namespace, use the matching **Mode**, or use a subscription token.  |
...
- Custom TLS options.
- Server-side subscriptions to map channels.
- Shared poll subscriptions.
- `getState` stream callbacks.
